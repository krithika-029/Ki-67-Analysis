        ...
        for idx in range(len(test_dataset)):
            if use_tta:
                ...
                for sample in tta_samples:
                    ...
                    tta_outputs.append(output.cpu().numpy().flatten()[0])
                ...
                avg_prob = output.cpu().numpy().flatten()[0]
...
def create_advanced_ensemble(results, ensemble_weights_path=None):
    ...
    conf_weights = all_confidence / np.sum(all_confidence, axis=1, keepdims=True)
    ...
    # 4. Adaptive threshold ensemble
    adaptive_threshold = np.percentile(weighted_probs, 75)  # Top 25% as positive
    adaptive_preds = (weighted_probs > adaptive_threshold).astype(int)
    
    # Adjust threshold to match class distribution
    ...
    # 5. High-confidence ensemble (only use samples where models agree)
    avg_confidence = np.mean(all_confidence, axis=1)
    high_conf_mask = avg_confidence > np.percentile(avg_confidence, 50)  # Top 50% confidence
    
    high_conf_probs = weighted_probs.copy()
    high_conf_preds = weighted_preds.copy()
    
    # For low-confidence samples, use more conservative approach
    low_conf_mask = ~high_conf_mask
    if np.sum(low_conf_mask) > 0:
        # Use majority vote for low-confidence samples
...
def plot_enhanced_results(individual_results, ensemble_results, save_path):
    ...
    n_ensembles = min(3, len(ensemble_results))  # Show top 3 ensembles
    ...
    # Best ensembles
    best_ensembles = sorted(ensemble_results.items(), key=lambda x: x[1]['accuracy'], reverse=True)[:n_ensembles]
    ...
    if 'confidence_scores' in individual_results['InceptionV3']:
